    ...
    return (
        <View style={styles.lastRow}>
            ...
            <TouchableOpacity style={styles.lessThan} disabled={(startObj?.startData == 1 || !isDataAvailable) ? true : false} onPress={handleNextPreviousPagePress.bind(null, 'back')}>
                <Image source={require('../assets/lessThan.png')} resizeMode={'contain'} style={[styles.iconStyle, { opacity: (startObj?.startData == 1 || !isDataAvailable) ? 0.3 : 1,width:15,height:15 }]} />
            </TouchableOpacity>
            <TouchableOpacity style={styles.lessThan} disabled={(endObj?.endData == dataLength || !isDataAvailable) ? true : false} onPress={handleNextPreviousPagePress.bind(null, 'next')}>
                <Image source={require('../assets/greaterThan.png')} resizeMode={'contain'} style={[styles.iconStyle, { opacity: (endObj?.endData == dataLength || !isDataAvailable) ? 0.3 : 1,width:15,height:15 }]} />
            </TouchableOpacity>
        </View>
    );
});
